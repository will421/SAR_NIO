...
public class MulticastEngine implements IMulticastEngine, AcceptCallback,
		ConnectCallback, DeliverCallback {

	enum ENGINE_STATE {
		NOTHINGNESS, CONNECT_TO_SERVER, CONNECT_TO_MEMBER, WORKING
	}

	private NioEngine nEngine; // NioEngine utilisé
	private IMulticastCallback callback; // Objet recevant les sorties du
											// multicast engine
	private NioChannel channelServer; // Channel de communication avec le
										// serveur de groupe
	...
	private void handleReceiveMember(ByteBuffer bytes) {
		...
		if (type == MESSAGE_TYPE.MESSAGE.ordinal()
				|| type == MESSAGE_TYPE.ADD_MEMBER.ordinal()) {
	...
	private void handleReceiveMessage(ByteBuffer bytes) {
		...
		MulticastQueueElement el = MulticastQueueElement.getElement(queue,
				clock, pidM);
		if (el == null) {
	...
	private void handleReceiveACK(ByteBuffer bytes) {
		...
		/*
		 * format : 1) type message 2) clock message 3) pid message 4) pidEnvoyeur
		 */
		bytes.position(0);
		int type = bytes.getInt();
		long clock = bytes.getLong();
	...
	public void deliver(NioChannel channel, ByteBuffer bytes) {
		if (channel == channelServer) {
			handleReceiveServer(bytes);
				...
				|| members.getChannelInLocal() == channel) {
			...
			if (type == MESSAGE_TYPE.ID) {
				int id = bytes.getInt();
				if (id == mPid) {
					if (members.getChannelInLocal() == null)
						members.setChannelInLocal(channel, mPid);
					else
	...
	public void closed(NioChannel channel) {
		...
		if(members.contains(channel) || members.getChannelInLocal()==channel)
		{
			int pid = members.disconnected(channel);
			callback.memberQuit(pid);
		}
		else if(channel == channelServer){
